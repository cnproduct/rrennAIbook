...
export interface CreaterrennAIbookOptions {
  bookPath: string;
  name?: string;
  description?: string;
  author?: string;
...
export interface CreaterrennAIbookResult {
  success: boolean;
  root: string;
  bookName: string;
  error?: string;
...
export function tryGitInit(root: string): boolean {
  try {
    ...
    execSync('git commit -m "Initial commit from rrennAIbook"', {
      stdio: "ignore",
      cwd: root,
    });
    return true;
 ...
 */
export async function createrrennAIbook(
  options: CreaterrennAIbookOptions,
): Promise<CreaterrennAIbookResult> {
  const {
    bookPath,
    name,
    description = "",
  ...
  // Create rrennAIbook.json
  const rrennAIbookJson = {
    name: bookName,
    version: "0.0.0",
    logo: "/logo.png",
  ...
  try {
    fs.writeFileSync(
      path.join(root, "rrennAIbook.json"),
      JSON.stringify(rrennAIbookJson, null, 2) + os.EOL,
    );
  } catch (err) {
    return {
      success: false,
      root,
      bookName,
      error: `Failed to write rrennAIbook.json: ${err}`,
    };
  }
