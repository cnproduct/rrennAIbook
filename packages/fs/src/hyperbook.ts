...
export const getJson = async (root: string): Promise<rrennAIbookJson> => {
  return fs
    .readFile(path.join(root, "rrennAIbook.json"))
    .then((f) => f.toString())
    .then(JSON.parse);
};

export const find = async (file: string): Promise<rrennAIbookJson> => {
  return findUp("rrennAIbook.json", {
    cwd: file,
  } as any)
    .then((f) => {
      if (!f) {
        throw new Error("Could not find rrennAIbook.json");
      }
...
export const makeRepoLink = (
  repoTemplate: rrennAIbookJson["repo"],
  vfile: VFile,
): string | null => {
...
export const getNavigationForFile = async (
  ...
  let current: rrennAIbookPage | null = null;
  let next: rrennAIbookPage | null = null;
  let previous: rrennAIbookPage | null = null;
  ...
  if (currentFile) {
    ...
    const resolveLink = (link: string): rrennAIbookPage | null => {
      if (!link) {
        return null;
      }
      if (link.startsWith("/@/")) {
...
export const getPagesAndSections = async (
  ...
  const rrennAIbook = await getJson(root);
  ...
  const getSectionsAndPages = async function ({
    ...
    let arrayOfPages: rrennAIbookPage[] = [];
    let arrayOfSections: rrennAIbookSection[] = [];

    for (const directory of directories) {
      let section: rrennAIbookSection = {
        name: directory.name,
        pages: [],
        sections: [],
        isEmpty: true,
      ...
      if (directory.index) {
        ...
        const repo = makeRepoLink(rrennAIbook.repo, directory.index);
        if (repo) {
          section.repo = repo;
        }
    ...
    for (const file of files) {
      ...
      const pageNavigation: PageNavigation | undefined =
        data.navigation === "default" || data.navigation === "hidden"
          ? data.navigation
          : undefined;
      const page: rrennAIbookPage = {
        ...data,
        isEmpty: file.markdown.content.trim() === "",
        navigation: pageNavigation,
        path: file.path,
      };
      const repo = makeRepoLink(rrennAIbook.repo, file);
      if (repo) {
        page.repo = repo;
      }
      if (file.path.href) {
    ...
    glossary: glossary.map(
      (g) =>
        ({
          ...
          repo: makeRepoLink(rrennAIbook.repo, g),
        }) as rrennAIbookPage,
    ),
  };
};
