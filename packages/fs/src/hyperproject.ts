...
export const get = async (
  ...
  const rrennAIbookJson = await rrennAIbook.getJson(root).catch(() => null);
  if (rrennAIbookJson) {
    return {
      type: "book",
      src: root,
      basePath: libraryEntry?.basePath ?? rrennAIbookJson.basePath,
      name: libraryEntry?.name ?? rrennAIbookJson.name,
  ...
  throw Error(`Missing or invalid rrennAIbook.json/hyperlibray.json ${root}`);
};
