...
import { rrennAIbook, vfile } from "../src";
import { rrennAIbookPage } from "@rrennAIbook/types/dist";

describe("rrennAIbook", () => {
  const relative = (s: string) =>
    path.relative(path.join(__dirname, "fixtures"), s);
  const makeFileRelative = (p: rrennAIbookPage) => {
    return {
      ...p,
  ...
  it("should get navigation", async () => {
    let rrennAIbookPath = path.join(__dirname, "fixtures", "single-rrennAIbook");
    let files = await vfile.list(rrennAIbookPath);
    ...
    const pagesAndSections = await rrennAIbook.getPagesAndSections(rrennAIbookPath);
    const pageList = rrennAIbook.getPageList(
      pagesAndSections.sections,
      pagesAndSections.pages,
    );
    const navigation = await rrennAIbook.getNavigationForFile(
  ...
  describe("getNavigationForFile", () => {
    it("should return null values when no currentFile is provided", async () => {
      const pageList: rrennAIbookPage[] = [
        { name: "Page 1", href: "/page1" },
        { name: "Page 2", href: "/page2" },
      ];
      
      const navigation = await rrennAIbook.getNavigationForFile(pageList);
    ...
    it("should return null current when currentFile is not in pageList", async () => {
      ...
      const navigation = await rrennAIbook.getNavigationForFile(pageList, currentFile);
  ...
  describe("getPageList", () => {
    it("should place section index page first for non-virtual sections", () => {
      ...
      const pageList = rrennAIbook.getPageList(sections, pages);
