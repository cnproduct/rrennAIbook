...
describe("list", () => {
  ...
  it("should get references", async () => {
    const files = await vfile.listForFolder(rrennAIbookPath, "glossary");
    expect(files.map(makeFileRelative)).toMatchSnapshot();
  });
});
...
describe("getMarkdown", () => {
  ...
  it("should render snippet with rrennAIbook", async () => {
    let files = await vfile.list(rrennAIbookPath);
    let parenthesesTest = files.find(
      (f) => f.path.relative === "rrennAIbook-test.md" && f.folder === "book",
    );
    if (!parenthesesTest) {
      throw Error("File not found");
    }
...
describe("getDirectory", () => {
  ...
  vfile.clean(rrennAIbookPath);
  it("should include main index", async () => {
    let directory = await vfile.getDirectory(rrennAIbookPath, "book");
