...
import { hyperproject, vfile, rrennAIbook, VFileBook, VFileGlossary } from "@rrennAIbook/fs";
...
import {
  ...
  rrennAIbookSection,
  Navigation,
} from "@rrennAIbook/types";
import lunr from "lunr";
import { process as rrennAIbookProcess } from "@rrennAIbook/markdown";
import packageJson from "./package.json";

export const ASSETS_FOLDER = "__rrennAIbook_assets";
...
export async function buildSingleBookPage(
  file: VFileBook,
  baseCtx: Pick<rrennAIbookContext, "config" | "makeUrl" | "project" | "root" | "version">,
  pageList: rrennAIbookPage[],
  pagesAndSections: Pick<Navigation, "pages" | "sections" | "glossary">,
  rootOut: string,
  assetsOut: string,
  rrennAIbookJson: rrennAIbookJson,
): Promise<SinglePageResult> {
  const n1 = await rrennAIbook.getNavigationForFile(pageList, file);
  ...
  const result = await rrennAIbookProcess(file.markdown.content, ctx);
  ...
  } else if (rrennAIbookJson.trailingSlash) {
...
export async function buildSingleGlossaryPage(
  file: VFileGlossary,
  baseCtx: Pick<rrennAIbookContext, "config" | "makeUrl" | "project" | "root" | "version">,  pageList: rrennAIbookPage[],
  pagesAndSections: Pick<Navigation, "pages" | "sections" | "glossary">,
  rootOut: string,
  assetsOut: string,
  rrennAIbookJson: rrennAIbookJson,
  ...
  if (rrennAIbookJson.trailingSlash) {
    href = path.posix.join(file.path.href || "", "index.html");
...
async function generateLlmsTxt(
  ...
  lines.push(`<SYSTEM>${rrennAIbookJson.name} - Version ${version}</SYSTEM>`);
  ...
  const processPage = async (page: rrennAIbookPage): Promise<void> => {
  ...
  lines.push(`
When you are writing a rrennAIbook syntax you must use markdown plus the documented elements, also keep track of colons. Colons act like parentheses in programming languages. Every opening colon must have a closing colon. For example, in the syntax :bold text: the first colon opens the bold formatting and the second colon closes it. If there is a missing colon, it can lead to formatting errors or unexpected behavior in the rendered output. Always ensure that colons are properly paired to maintain the intended structure and appearance of your rrennAIbook content.
...
export async function runBuildProject(
  ...
  if (project.type === "book") {
    ...
    await rimraf(path.join(out, ".rrennAIbook", "out"));
...
export function makeBaseCtx(
  ...
  rootProject: Hyperproject,
): Pick<rrennAIbookContext, "config" | "makeUrl" | "project" | "root" | "version"> {
  const resolveRelativePath = (p: string, page: rrennAIbookPage): string => {
    if (p.startsWith("/")) {
      return p;
    }
...
async function runBuild(
  ...
  console.log(`${chalk.blue(`[${prefix}]`)} Reading rrennAIbook.json.`);
  const rrennAIbookJson = await rrennAIbook.getJson(root);

  if (!basePath && rrennAIbookJson?.basePath) {
    basePath = rrennAIbookJson.basePath;
  }
  ...
  rrennAIbookJson.basePath = basePath;

  vfile.clean(root);

  let link: Link | undefined = undefined;
  if (rootProject.type === "library") {
    link = await hyperproject.getLink(rootProject, rrennAIbookJson.language);
  }
  if (link) {
    if (!rrennAIbookJson.links) {
      rrennAIbookJson.links = [link];
    } else {
      rrennAIbookJson.links.push(link);
    }
  }

  let rootOut = path.join(root, ".rrennAIbook", "out");
  if (out) {
    rootOut = path.join(out, ".rrennAIbook", "out", basePath || "");
  }
  ...
  const baseCtx = makeBaseCtx(root, rrennAIbookJson, basePath, rootProject);

  const directives = new Set<string>([]);
  const pagesAndSections = await rrennAIbook.getPagesAndSections(root);
  ...
  if (!faviconExists && rrennAIbookJson.logo) {
    ...
    if (!rrennAIbookJson.logo.includes("://")) {
      ...
      if (rrennAIbookJson.logo.startsWith("/")) {
        ...
        const rootPath = path.join(root, rrennAIbookJson.logo);
        const bookPath = path.join(root, "book", rrennAIbookJson.logo);
        const publicPath = path.join(root, "public", rrennAIbookJson.logo);
      ...
      if (logoPath) {
        ...
        console.log(
          `${chalk.yellow(`[${prefix}]`)} Warning: Could not generate favicons. Logo file not found: ${rrennAIbookJson.logo}`,
        );
      }
    }
  }
  ...
  for (let asset of mainAssets) {
      ...
      await cp(assetPath, assetOut, {
        ...
        filter: (src) => {
          if (src.includes("lunr-languages")) {
            return (
              rrennAIbookJson.language !== undefined &&
              rrennAIbookJson.language !== "en" &&
              (src.endsWith("lunr-languages") ||
                src.endsWith(`lunr.${rrennAIbookJson.language}.min.js`) ||
    ...
    process.stdout.write(
      `${chalk.blue(`[${prefix}]`)} Copying rrennAIbook assets: [${i++}/${mainAssets.length}]`,
    );
    if (process.env.CI) {
      process.stdout.write("\n");
    }
  ...
  if (rrennAIbookJson.search) {
    const documents: Record<string, any> = {};
    console.log(`${chalk.blue(`[${prefix}]`)} Building search index`);

    let foundLanguage = false;
    if (rrennAIbookJson.language && rrennAIbookJson.language !== "en") {
      try {
        // Only register lunr language plugins once to avoid "Overwriting" warnings
        if (!(lunr as any)["_rrennAIbook_lang_loaded_" + rrennAIbookJson.language]) {
          require("./lunr-languages/lunr.stemmer.support.min.js")(lunr);
          require(`./lunr-languages/lunr.${rrennAIbookJson.language}.min.js`)(lunr);
          (lunr as any)["_rrennAIbook_lang_loaded_" + rrennAIbookJson.language] = true;
        }
        foundLanguage = true;
      } catch (e) {
        console.log(e);
        console.log(
          `${chalk.yellow(`[${prefix}]`)} ${rrennAIbookJson.language} is not a valid value for the language key. See https://github.com/MihaiValentin/lunr-languages for possible values. Falling back to English.`,
        );
      }
    }
    const idx = lunr(function () {
      if (foundLanguage) {
        // @ts-ignore
        this.use(lunr[rrennAIbookJson.language]);
      }
      this.ref("href");
...
`;
  ...
  let language = "en";
  if (
    rrennAIbookJson.language &&
    supportLanguages.includes(rrennAIbookJson.language)
  ) {
    language = rrennAIbookJson.language;
  }
