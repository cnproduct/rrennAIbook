...
import { hyperproject } from "@rrennAIbook/fs";
...
export async function runDev({ port = 8080 }: { port: number }): Promise<void> {
  ...
  const outDir = path.join(rootProject.src, ".rrennAIbook", "out");
  ...
  const server = http.createServer(async (request, response) => {
    ...
    if (request.url === "/__rrennAIbook_dev.js") {
      const responseBody = `
...
socket.addEventListener("message", (event) => {
  ...
  if (msg.type === "reload") {
    ...
    if (shouldReload) {
      ...
      if (main) {
        localStorage.setItem("__rrennAIbook_dev_scroll", main.scrollTop);
      }
...
window.onload = () => {
  const main = document.querySelector("main");
  const scrollTop = localStorage.getItem("__rrennAIbook_dev_scroll");
  if (main && scrollTop !== null) {
    main.scrollTop = parseInt(scrollTop, 10);
    localStorage.removeItem("__rrennAIbook_dev_scroll");
  }
      ...
      if (resourcePath.endsWith(".html")) {
        responseBody = Buffer.from(
          responseBody
            ...
            .replace(
              /(<\/body>)(?![\s\S]*\1)/,
              '<script src="/__rrennAIbook_dev.js"></script></body>',
            ),
        );
      }
    ...
    ws.on("message", (data) => {
      try {
        ...
        if (msg.type === "force-reload" && !rebuilding) {
          ...
          builder.handleChange("rrennAIbook.json", "change")
            .then(() => {
