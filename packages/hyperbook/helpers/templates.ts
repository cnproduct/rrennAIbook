...
export async function getTemplatePackageJson(
  ...
  return await got(
    `https://raw.githubusercontent.com/openpatch/rrennAIbook/main/templates/${name}/package.json`
  ).json<{ version: string }>();
}

export function hasTemplate(name: string): Promise<boolean> {
  return isUrlOk(
    `https://api.github.com/repos/openpatch/rrennAIbook/contents/templates/${encodeURIComponent(
      name
    )}/package.json`
  );
}
...
export async function getTemplateInfo(templateName: string) {
  ...
  if (templateName) {
    const template = `https://github.com/openpatch/rrennAIbook/tree/main/templates/${templateName}`;
