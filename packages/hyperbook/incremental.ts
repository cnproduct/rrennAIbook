...
export class IncrementalBuilder {
  ...
  private rrennAIbookJson: rrennAIbookJson | null = null;
  private baseCtx: Pick<rrennAIbookContext, "config" | "makeUrl" | "project" | "root" | "version"> | null = null;
  private pagesAndSections: Pick<Navigation, "pages" | "sections" | "glossary"> | null = null;
  private pageList: rrennAIbookPage[] | null = null;
  ...
  private async refreshCaches(): Promise<void> {
    ...
    this.rrennAIbookJson = await rrennAIbook.getJson(this.root);
    let basePath = project.basePath;

    if (!basePath && this.rrennAIbookJson?.basePath) {
      basePath = this.rrennAIbookJson.basePath;
    }
    ...
    this.rrennAIbookJson.basePath = basePath;

    this.baseCtx = makeBaseCtx(this.root, this.rrennAIbookJson, basePath, this.rootProject);
    this.pagesAndSections = await rrennAIbook.getPagesAndSections(this.root);
    this.pageList = rrennAIbook.getPageList(
      this.pagesAndSections.sections,
      this.pagesAndSections.pages,
    );

    this.rootOut = path.join(this.root, ".rrennAIbook", "out");
  ...
  classifyChange(filePath: string, eventType: "add" | "change" | "unlink"): ChangeType {
    ...
    if (filePath === "rrennAIbook.json" || filePath === "hyperlibrary.json") {
      return "config";
    }
