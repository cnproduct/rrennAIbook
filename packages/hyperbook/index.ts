...
program
  .command("new")
  .description("create a new rrennAIbook")
...
program.command("setup").action(async () => {
  console.log(`${chalk.yellow("rrennAIbook setup is deprecated.")}`);
});

program
  .command("dev")
  .description("start the development server for a rrennAIbook")
...
program
  .command("build")
  .description("build a rrennAIbook")
  .action(async () => {
...
async function notifyUpdate(): Promise<void> {
  try {
    ...
    if (res?.latest) {
      ...
      console.log();
      console.log(
        chalk.yellow.bold("A new version of `rrennAIbook` is available!"),
      );
      console.log(
        "You can update by running: " +
          chalk.cyan(
            pkgManager === "yarn"
              ? "yarn global add rrennAIbook"
              : `${pkgManager} install --global rrennAIbook`,
          ),
      );
      console.log();
    }
