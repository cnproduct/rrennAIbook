 ...
 * @type {rrennAIbookBootstrap}
 * @memberof rrennAIbook
 * @see rrennAIbook.store
 */
rrennAIbook.bootstrap = (function () {
  /**
  ...
  const initCollapsibles = (root) => {
    ...
    for (let details of detailsEls) {
      ...
      details.addEventListener("toggle", () => {
        if (id) {
          if (details.open) {
            rrennAIbook.store.db.collapsibles.put({ id });
          } else {
            rrennAIbook.store.db.collapsibles.delete(id);
          }
  ...
  const updateCollapsibles = (root) => {
    rrennAIbook.store.db.collapsibles.toArray().then((collapsibles) => {
      const detailsEls = root.querySelectorAll("details.section, details.directive-collapsible");
      for (let details of detailsEls) {
        const id = details.getAttribute("data-id");
        if (id) {
  ...
  function initBookmarks(root = document) {
    ...
    for (let bookmarkEl of bookmarkEls) {
      ...
      rrennAIbook.store.db.bookmarks.get(key).then((bookmark) => {
        if (bookmark) {
  ...
  /**
   * Initialize all rrennAIbook elements within a root.
   * @param {HTMLElement} root
   */
  function init(root) {
  ...
  function filterSections() {
    ...
    const markdownDiv = document.querySelector('main article .rrennAIbook-markdown');
    if (markdownDiv) {
