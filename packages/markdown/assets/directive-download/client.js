 ...
 * @type {rrennAIbookDownload}
 * @memberof rrennAIbook
 * @see rrennAIbook.i18n
 */
rrennAIbook.download = (function () {
  const init = (root) => {
    ...
    for (let el of els) {
      ...
      fetch(src).then((r) => {
        ...
        if (isOnline) {
          ...
          labelEl.innerHTML.replace(`(${rrennAIbook.i18n.get("download-offline")})`, "");
        } else {
          labelEl.classList.add("offline");
          labelEl.innerHTML += ` (${rrennAIbook.i18n.get("download-offline")})`;
        }
      });
    }
  };
