...
rrennAIbook.embed = rrennAIbook.embed || {};

rrennAIbook.embed.consent = (function () {
  ...
  async function isAllowed(consentId) {
    try {
      var entry = await rrennAIbook.store.db.consent.get(consentId);
  ...
  async function allow(consentId) {
    await rrennAIbook.store.db.consent.put({ id: consentId, allowed: true });
  }
