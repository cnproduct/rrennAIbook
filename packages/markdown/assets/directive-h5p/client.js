...
rrennAIbook.h5p = (function () {
  ...
  const save = (id) =>
    H5P.getUserData(id, "state", (error, userData) => {
      if (!error) {
        rrennAIbook.store.db.h5p.put({ id, userData });
      }
    });

  const init = async (root) => {
    const els = root.getElementsByClassName("directive-h5p");
    const assets = `${rrennAIbook_ASSETS}directive-h5p`;
    ...
    for (const el of els) {
      ...
      if (h5pFrame && src) {
        const result = await rrennAIbook.store.db.h5p.get(id);
        const h5pOptions = {
          ...h5pBaseOptions,
          id,
          h5pJsonPath: src,
