...
rrennAIbook.p5 = (function () {
  ...
  function initElement(elem) {
    ...
    resetEl?.addEventListener("click", () => {
      rrennAIbook.store.db.p5.delete(id);
    ...
    editor?.addEventListener("code-input_load", async () => {
      if (id) {
        const result = await rrennAIbook.store.db.p5.get(id);
        if (result) {
        ...
        editor.addEventListener("input", () => {
          rrennAIbook.store.db.p5.put({ id, sketch: editor.value });
        });
      }
