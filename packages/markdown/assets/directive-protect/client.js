 ...
 * @type {rrennAIbookProtect}
 * @memberof rrennAIbook
 * @see rrennAIbook.store
 */
rrennAIbook.protect = (function () {
  /**
  ...
  function onInputToast(inputEl, el) {
    ...
    rrennAIbook.store.db.protect.put({ id, passwordHash: btoa(value) }).then(() => {
  ...
  function onUpdateToast(inputEl, el, hiddenEl) {
    const id = el.getAttribute("data-id");
    rrennAIbook.store.db.protect.get(id).then((result) => {
      if (result) {
