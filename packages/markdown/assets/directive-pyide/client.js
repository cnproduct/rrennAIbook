...
rrennAIbook.python = (function () {
  ...
  const pyodideWorker = new Worker(
    `${rrennAIbook_ASSETS}directive-pyide/webworker.js`
  );
    ...
    for (let elem of elems) {
      ...
      if (callback) {
        if (elem.id === id && type === "run") {
          if (window.crossOriginIsolated) {
            run.textContent = rrennAIbook.i18n.get("pyide-running-click-to-stop");
            run.addEventListener("click", interruptExecution);
          } else {
            run.textContent = rrennAIbook.i18n.get("pyide-running-refresh-to-stop");

            run.addEventListener("click", reload);
          }
        } else if (test && elem.id === id && type === "test") {
          if (window.crossOriginIsolated) {
            test.textContent = rrennAIbook.i18n.get("pyide-testing-click-to-stop");
            test.addEventListener("click", interruptExecution);
          } else {
            test.textContent = rrennAIbook.i18n.get("pyide-testing-refresh-to-stop");
  ...
  const init = (root) => {
    ...
    for (let elem of elems) {
      ...
      resetEl?.addEventListener("click", () => {
        rrennAIbook.store.db.pyide.delete(id);
        window.location.reload();
      });
      ...
      editor.addEventListener("code-input_load", async () => {
        const result = await rrennAIbook.store.db.pyide.get(id);
        if (result) {
      ...
      editor.addEventListener("input", () => {
        rrennAIbook.store.db.pyide.put({ id, script: editor.value });
      });
