 ...
 * @type {rrennAIbookSlideshow}
 * @memberof rrennAIbook
 * @see rrennAIbook.store
 */
rrennAIbook.slideshow = (function () {
  /**
   * @param {string} id
   */
  const update = (id) => {
    rrennAIbook.store.db.slideshow.get(id).then((result) => {
  ...
  const moveBy = (id, steps) => {
    ...
    rrennAIbook.store.db.slideshow.get(id).then((result) => {
      ...
      rrennAIbook.store.db.slideshow.put({ id, active }).then(() => {
        window.rrennAIbook.slideshow.update(id);
      });
    });
  };
  ...
  const setActive = (id, index) => {
    ...
    if (index >= 0 && index < images.length) {
      rrennAIbook.store.db.slideshow.put({ id, active: index }).then(() => {
        window.rrennAIbook.slideshow.update(id);
      });
    }
  };
