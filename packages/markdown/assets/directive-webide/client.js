...
rrennAIbook.webide = (function () {
  ...
  function initElement(elem) {
    ...
    resetEl?.addEventListener("click", () => {
      if (window.confirm(rrennAIbook.i18n.get("webide-reset-prompt"))) {
        rrennAIbook.store.db.webide.delete(id);
    ...
    const load = async () => {
      const result = await rrennAIbook.store.db.webide.get(id);
      if (!result) {
        return;
      }
    ...
    const update = () => {
      rrennAIbook.store.db.webide.put({
        id,
