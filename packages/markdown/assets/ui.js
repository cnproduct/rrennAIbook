...
rrennAIbook.ui = (function () {
  ...
  function toggleBookmark(key, label) {
    ...
    rrennAIbook.store.db.bookmarks.get(key).then((bookmark) => {
      if (!bookmark) {
        rrennAIbook.store.db.bookmarks.add({ path: key, label }).then(() => {
          el.forEach((e) => e.classList.add("active"));
          rrennAIbook.bookmarks.update();
        });
      } else {
        rrennAIbook.store.db.bookmarks.delete(key).then(() => {
 ...
 * @type {rrennAIbookQrcode}
 * @memberof rrennAIbook
 */
rrennAIbook.qrcode = (function () {
  /**
 ...
 * @type {rrennAIbookShare}
 * @memberof rrennAIbook
 * @see rrennAIbook.i18n
 */
rrennAIbook.share = (function () {
  /**
  ...
  function copyUrl() {
    ...
    navigator.clipboard.writeText(url).then(() => {
      ...
      button.textContent = rrennAIbook.i18n.get("share-dialog-url-copied");
