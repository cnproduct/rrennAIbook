...
import { registerBasicHelpers } from "@rrennAIbook/fs";
...
const clusterPages = (
  pages: rrennAIbookPage[],
): Record<string, rrennAIbookPage[]> => {
  const cluster: Record<string, rrennAIbookPage[]> = {};
  ...
  return (tree: Root, file: VFile) => {
    ...
    visit(tree, function (node) {
      if (isDirective(node)) {
        ...
        let filteredPages = pageList.filter((p) => evaluateQuery(query, p as rrennAIbookPage & Record<string, unknown>));
