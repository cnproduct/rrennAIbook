...
// ===== rrennAIbook Management =====

router.get("/rrennAIbooks", async function (req, res) {
  try {
    ...
    if (req.user.role === "teacher") {
      ...
      rrennAIbooks = rrennAIbooks.filter(function (hb) {
        return permSet["rrennAIbook:" + hb.id + ":read"];
      });
    }
    res.json({ rrennAIbooks: rrennAIbooks });
  } catch (error) {
...
router.post("/rrennAIbooks", async function (req, res) {
  try {
    var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbooks:create");
    if (!allowed) {
    ...
    // Grant the creator full permissions on the new rrennAIbook if they are a teacher
    if (req.user.role === "teacher") {
    ...
    if (error instanceof Error && error.message.includes("UNIQUE constraint failed")) {
      res.status(409).json({ error: "rrennAIbook slug already exists" });
      return;
    }
...
router.put("/rrennAIbooks/:id", async function (req, res) {
  try {
    var id = req.params.id;
    var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + id + ":update");
    if (!allowed) {
      res.status(403).json({ error: "Permission denied" });
      return;
    }
...
router.delete("/rrennAIbooks/:id", async function (req, res) {
  try {
    var id = req.params.id;
    var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + id + ":delete");
    if (!allowed) {
      res.status(403).json({ error: "Permission denied" });
      return;
    }
...
router.get("/rrennAIbooks/:rrennAIbookId/groups", async function (req, res) {
  try {
    var rrennAIbookId = req.params.rrennAIbookId;
    var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + rrennAIbookId + ":read");
    if (!allowed) {
      res.status(403).json({ error: "Permission denied" });
      return;
    }
...
router.post("/rrennAIbooks/:rrennAIbookId/groups", async function (req, res) {
  try {
    var rrennAIbookId = req.params.rrennAIbookId;
    var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + rrennAIbookId + ":groups:create");
    if (!allowed) {
      res.status(403).json({ error: "Permission denied" });
      return;
    }
    ...
    if (error instanceof Error && error.message.includes("UNIQUE constraint failed")) {
      res
        .status(409)
        .json({ error: "Group name already exists in this rrennAIbook" });
      return;
    }
...
router.put("/groups/:id", async function (req, res) {
  try {
    var id = req.params.id;
    var group = await db.getAsync("SELECT rrennAIbook_id FROM groups WHERE id = ?", [id]);
    if (!group) {
      res.status(404).json({ error: "Group not found" });
      return;
    }
    var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + group.rrennAIbook_id + ":groups:update");
    if (!allowed) {
      res.status(403).json({ error: "Permission denied" });
      return;
    }
...
router.delete("/groups/:id", async function (req, res) {
  try {
    ...
    var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + group.rrennAIbook_id + ":groups:delete");
    if (!allowed) {
      res.status(403).json({ error: "Permission denied" });
      return;
    }
...
// Helper to resolve rrennAIbook_id from a groupId
async function getrrennAIbookIdForGroup(groupId) {
  var group = await db.getAsync("SELECT rrennAIbook_id FROM groups WHERE id = ?", [groupId]);
  return group ? group.rrennAIbook_id : null;
}
...
router.get("/students", async function (req, res) {
  try {
    ...
    if (groupId) {
      var hbId = await getrrennAIbookIdForGroup(groupId);
      if (hbId) {
        var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + hbId + ":read");
        if (!allowed) {
          res.status(403).json({ error: "Permission denied" });
          return;
        }
    ...
    for (var i = 0; i < students.length; i++) {
      ...
      var hbId = null;
      if (groupId) {
        var grp = await db.getAsync("SELECT rrennAIbook_id FROM groups WHERE id = ?", [groupId]);
        hbId = grp ? grp.rrennAIbook_id : null;
      } else if (student.group_id) {
        var grp2 = await db.getAsync("SELECT rrennAIbook_id FROM groups WHERE id = ?", [student.group_id]);
        hbId = grp2 ? grp2.rrennAIbook_id : null;
      }
...
router.post("/students", async function (req, res) {
  try {
    ...
    if (groupId) {
      var hbId = await getrrennAIbookIdForGroup(groupId);
      if (hbId) {
        var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + hbId + ":students:create");
        if (!allowed) {
          res.status(403).json({ error: "Permission denied" });
          return;
        }
...
router.put("/students/:id/password", async function (req, res) {
  try {
    ...
    if (student.group_id) {
      var hbId = await getrrennAIbookIdForGroup(student.group_id);
      if (hbId) {
        var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + hbId + ":students:update");
        if (!allowed) {
          res.status(403).json({ error: "Permission denied" });
          return;
        }
...
router.delete("/students/:id", async function (req, res) {
  try {
    ...
    if (student && student.group_id) {
      var hbId = await getrrennAIbookIdForGroup(student.group_id);
      if (hbId) {
        var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + hbId + ":students:delete");
        if (!allowed) {
          res.status(403).json({ error: "Permission denied" });
          return;
        }
...
router.get("/students/:studentId/store", async function (req, res) {
  try {
    ...
    var group = await db.getAsync("SELECT rrennAIbook_id FROM groups WHERE id = ?", [
      student.group_id,
    ]);
    ...
    var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + group.rrennAIbook_id + ":read");
    if (!allowed) {
      res.status(403).json({ error: "Permission denied" });
      return;
    }

    var state = await db.reconstructState(student.id, group.rrennAIbook_id);
...
router.post("/impersonate/:studentId", async function (req, res) {
  try {
    ...
    var allowed = await middleware.checkPermission(req.user.id, req.user.role, "rrennAIbook:" + groupInfo.rrennAIbook_id + ":read");
    if (!allowed) {
      res.status(403).json({ error: "Permission denied" });
      return;
    }
