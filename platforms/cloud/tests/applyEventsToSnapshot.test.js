...
function getTablesFromWrapper(snapshot) {
  return snapshot.data.rrennAIbook.data.tables;
}
...
describe("applyEventsToSnapshot", () => {
  describe("with wrapper format (from sendSnapshot)", () => {
    ...
    it("should preserve the wrapper structure after applying events", () => {
      ...
      expect(result.data.rrennAIbook).toBeDefined();
      expect(result.data.rrennAIbook.formatName).toBe("dexie");
      expect(result.data.rrennAIbook.data.tables).toBeDefined();
      expect(result.data.rrennAIbook.data.tables.length).toBeGreaterThan(0);
    });
  });
  ...
  describe("with default empty snapshot (from reconstructState)", () => {
    it("should apply events to the default empty snapshot", () => {
      ...
      expect(result.data.rrennAIbook.data.tables).toBeDefined();
      const table = result.data.rrennAIbook.data.tables.find(
  ...
  describe("full roundtrip simulation", () => {
    it("should produce state that loadFromCloud can consume", () => {
      ...
      // loadFromCloud does: data.snapshot.data → storeData, then storeData.rrennAIbook
      ...
      expect(rrennAIbook.formatName).toBe("dexie");
      expect(rrennAIbook.data.tables).toBeDefined();

      const collapsibles = rrennAIbook.data.tables.find(
        (t) => t.name === "collapsibles",
      );
      ...
      const tabs = rrennAIbook.data.tables.find((t) => t.name === "tabs");
      expect(tabs).toBeDefined();
      expect(tabs.rows).toHaveLength(1);
    });

    it("should produce state that matches a full snapshot from sendSnapshot", () => {
      // Simulate: sendSnapshot wraps as { version: 1, data: { rrennAIbook: dexieExport } }
      ...
      // Verify structure
      const rrennAIbook = result.data.rrennAIbook;
      expect(rrennAIbook).toBeDefined();
  ...
  describe("with REAL Dexie export format (data[] array separate from tables[])", () => {
    ...
    function findDataEntry(snapshot, tableName) {
      return snapshot.data.rrennAIbook.data.data.find(
    ...
    it("should produce correct structure for loadFromCloud with real Dexie format", () => {
      ...
      expect(rrennAIbook).toBeDefined();
      expect(rrennAIbook.data.data).toBeDefined(); // real Dexie format

      const collData = rrennAIbook.data.data.find(
        (d) => d.tableName === "collapsibles",
      );
      expect(collData.rows[0].collapsed).toBe(true);

      const sqlData = rrennAIbook.data.data.find(
    ...
    it("should handle snapshot with many empty tables (real-world scenario)", () => {
      ...
      const result = applyEventsToSnapshot(snapshot, events);
      const sqlData = result.data.rrennAIbook.data.data.find(
        (d) => d.tableName === "sqlideScripts",
      );
