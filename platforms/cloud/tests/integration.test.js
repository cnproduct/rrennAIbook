...
function createTestDb() {
  ...
  // Seed test data
  db.prepare(
    "INSERT INTO rrennAIbooks (slug, name) VALUES ('test', 'Test rrennAIbook')",
  ).run();
  db.prepare(
    "INSERT INTO groups (rrennAIbook_id, name) VALUES (1, 'Test Group')",
  ).run();
  db.prepare(
...
function applyEventsToSnapshot(snapshotData, events) {
  ...
  var dexieData =
    snapshotData.data && snapshotData.data.rrennAIbook
...
function makeHelpers(db) {
  ...
  function getEventsSince(userId, rrennAIbookId, afterEventId) {
    return db
      ...
      .all(userId, rrennAIbookId, afterEventId);
  }

  function getLatestEventId(userId, rrennAIbookId) {
    var row = db
      .prepare(
        "SELECT MAX(id) as max_id FROM events WHERE user_id = ? AND rrennAIbook_id = ?",
      )
      .get(userId, rrennAIbookId);
  ...
  function replaceWithSnapshot(userId, rrennAIbookId, data) {
    var replaceAll = db.transaction(function () {
      ...
      var result = db
        .prepare(
          "INSERT INTO snapshots (user_id, rrennAIbook_id, data, last_event_id) VALUES (?, ?, ?, 0)",
        )
  ...
  function reconstructState(userId, rrennAIbookId) {
    var snapshot = getLatestSnapshot(userId, rrennAIbookId);
    var snapshotEventId = snapshot ? snapshot.last_event_id : 0;
    var events = getEventsSince(userId, rrennAIbookId, snapshotEventId);
...
describe("integration: event-sourcing full flow", () => {
  ...
  it("should reconstruct state from events only (no snapshot)", () => {
    ...
    const lastEventId = helpers.appendEvents(userId, rrennAIbookId, [
      {
        table: "collapsibles",
        op: "create",
        primKey: "s1",
    ...
    const state = helpers.reconstructState(userId, rrennAIbookId);
    ...
    expect(state.data.data.rrennAIbook).toBeDefined();

    const tables = state.data.data.rrennAIbook.data.tables;
  ...
  it("should reconstruct state from snapshot + events", () => {
    ...
    helpers.replaceWithSnapshot(userId, rrennAIbookId, snapshotData);
  ...
  it("should simulate full client lifecycle: login → events → reload", () => {
    ...
    let state = helpers.reconstructState(userId, rrennAIbookId);
    expect(state).toBeNull();

    // 2. User interacts → Dexie hooks capture events → client sends batch
    helpers.appendEvents(userId, rrennAIbookId, [
      {
        table: "collapsibles",
        op: "create",
    ...
    state = helpers.reconstructState(userId, rrennAIbookId);
    expect(state).not.toBeNull();

    // 4. Simulate what loadFromCloud does:
    //    const storeData = data.snapshot.data || data.snapshot;
    //    const { rrennAIbook } = storeData;
    const apiResponse = {
      snapshot: state.data,
      lastEventId: state.lastEventId,
    };
    const storeData = apiResponse.snapshot.data || apiResponse.snapshot;
    const rrennAIbook = storeData.rrennAIbook;

    expect(rrennAIbook).toBeDefined();
    expect(rrennAIbook.data.tables).toBeDefined();

    const coll = rrennAIbook.data.tables.find(
  ...
  it("should handle events after reload (second batch)", () => {
    // First batch
    const firstBatchLastId = helpers.appendEvents(userId, rrennAIbookId, [
      {
        table: "collapsibles",
        op: "create",
        primKey: "s1",
    ...
    const serverLatest = helpers.getLatestEventId(userId, rrennAIbookId);
  ...
  it("should handle snapshot with no events (just snapshot)", () => {
    ...
    const state = helpers.reconstructState(userId, rrennAIbookId);
    const bookmarks = state.data.data.rrennAIbook.data.tables.find(
      (t) => t.name === "bookmarks",
    );
