...
import { createrrennAIbook, CreaterrennAIbookOptions } from "create-rrennAIbook";
...
const LICENSE_OPTIONS = [
  ...
  {
    label: "Creative Commons Attribution-NonCommercial-NoDervis (CC BY-NC-ND)",
    value: "cc-by-nc-nd",
  },
...
export async function createNewrrennAIbook(context: vscode.ExtensionContext) {
  ...
  async function collectInputs() {
    ...
    const folderUris = await vscode.window.showOpenDialog({
      ...
      title: "Select folder where the rrennAIbook will be created",
    });
    ...
    const bookNameInput = await vscode.window.showInputBox({
      title: `Create rrennAIbook (${state.step}/${state.totalSteps})`,
    ...
    state.step = 6;
    const licenseSelection = await vscode.window.showQuickPick(
      LICENSE_OPTIONS,
      {
        title: `Create rrennAIbook (${state.step}/${state.totalSteps})`,
  ...
  // Create the rrennAIbook
  await vscode.window.withProgress(
    {
      location: vscode.ProgressLocation.Notification,
      title: "Creating rrennAIbook",
      cancellable: false,
    },
    async (progress) => {
      progress.report({ message: "Setting up project..." });

      const options: CreaterrennAIbookOptions = {
      ...
      const result = await createrrennAIbook(options);

      if (!result.success) {
        vscode.window.showErrorMessage(
          `Failed to create rrennAIbook: ${result.error}`,
        );
        return;
      }
      ...
      const choice = await vscode.window.showInformationMessage(
        `rrennAIbook "${result.bookName}" created successfully at ${result.root}`,
        openFolder,
      );
