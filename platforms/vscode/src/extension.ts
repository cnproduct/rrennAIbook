...
import { rrennAIbook } from "@rrennAIbook/fs";
import { createNewrrennAIbook } from "./NewCommand";
import { rrennAIbookFoldingProvider } from "./FoldingProvider";
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let disposableDevServer = vscode.commands.registerCommand(
    "rrennAIbook.startDevServer",
    async () => {
      const terminal = vscode.window.createTerminal("rrennAIbook");
      terminal.show();
      terminal.sendText("npx rrennAIbook dev");
    },
  );

  // Command: Open index.md
  let disposableOpenIndexMd = vscode.commands.registerCommand(
    "rrennAIbook.openIndexMd",
    async () => {
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (!workspaceFolders) {
        vscode.window.showErrorMessage(
          "No workspace folder open. Please open a rrennAIbook project first.",
        );
        return;
      }

      const workspaceFolder = workspaceFolders[0];
      const rrennAIbookRoot = await rrennAIbook.findRoot(
        workspaceFolder.uri.fsPath,
      );

      if (!rrennAIbookRoot) {
        vscode.window.showErrorMessage(
          "No rrennAIbook project found. Please create one first using 'Create new rrennAIbook'.",
        );
        return;
      }

      const indexPath = path.join(rrennAIbookRoot, "book", "index.md");
      try {
  ...
  let disposableCreateNewPage = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      const bookPath = path.join(rrennAIbookRoot, "book");
...
`;
  ...
  let disposableOpenDocumentation = vscode.commands.registerCommand(
    "rrennAIbook.openDocumentation",
    () => {
      vscode.env.openExternal(
        vscode.Uri.parse("https://rrennAIbook.openpatch.org"),
      );
    },
  );
  ...
  let disposableNew = vscode.commands.registerCommand(
    "rrennAIbook.new",
    async () => {
      await createNewrrennAIbook(context);
    },
  );
  ...
  // Helper function to get rrennAIbook root with caching
  const rootCache = new Map<string, string>();
  async function getrrennAIbookRoot(
    docPath: string,
  ): Promise<string | undefined> {
    if (rootCache.has(docPath)) {
      return rootCache.get(docPath);
    }
    const root = await rrennAIbook.findRoot(docPath);
    if (root) {
  ...
  const bookProvider = vscode.languages.registerCompletionItemProvider(
    DocumentSelectorMarkdown,
    {
      async provideCompletionItems(document, position) {
        ...
        const workspaceFolder = await getrrennAIbookRoot(document.uri.path);
        if (!workspaceFolder) {
          return undefined;
        }
  ...
  // Register folding provider for rrennAIbook directive blocks
  const foldingProvider = vscode.languages.registerFoldingRangeProvider(
    [
      { language: "markdown", scheme: "file" },
      { language: "handlebars", scheme: "file" },
    ],
    new rrennAIbookFoldingProvider(),
  );
