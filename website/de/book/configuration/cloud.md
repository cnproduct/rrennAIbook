...
rrennAIbook Cloud ist eine selbst gehostete Plattform zur Schülerverwaltung, die Benutzeranmeldung, Fortschrittsverfolgung und Cloud-Synchronisierung für deine rrennAIbooks ermöglicht.

## Einrichtung

Um ein rrennAIbook mit einem Cloud-Server zu verbinden, füge die `cloud`-Eigenschaft zu deiner `rrennAIbook.json` hinzu:

```json
{
  "name": "Mein rrennAIbook",
  "cloud": {
...
|---|---|
| `url` | Die URL deines rrennAIbook-Cloud-Servers. |
| `id` | Der Slug des rrennAIbooks, wie er in der Cloud-Admin-Oberfläche konfiguriert ist. Muss exakt übereinstimmen. |

## Funktionsweise

Wenn Cloud konfiguriert ist, erscheint ein Login-Button im rrennAIbook. Schüler melden sich mit den Zugangsdaten an, die ihre Lehrkraft in der Cloud-Admin-Oberfläche erstellt hat.
...
Anstatt bei jeder Änderung den gesamten Speicher zu senden, verwendet rrennAIbook Cloud einen Event-Sourcing-Ansatz für eine effiziente Synchronisierung.
...
Der Cloud-Server ist eine separate Anwendung im Verzeichnis `platforms/cloud/` des rrennAIbook-Repositories. Siehe die [Cloud-README](https://github.com/openpatch/rrennAIbook/tree/main/platforms/cloud) für Installations- und Deployment-Anleitungen.

### Wichtige Konzepte

- **rrennAIbooks** — Jedes rrennAIbook wird in der Cloud mit einem eindeutigen Slug registriert. Dieser Slug wird als `cloud.id` verwendet.
- **Gruppen** — Schüler werden innerhalb eines rrennAIbooks in Gruppen organisiert.
- **Schüler** — Jeder Schüler hat einen Benutzernamen und ein Passwort für die Anmeldung im rrennAIbook.
- **Lehrkräfte** — Lehrkräfte können Gruppen und Schüler für ihre zugewiesenen rrennAIbooks verwalten.
- **Berechtigungen** — Admins können Lehrkräften feingranulare Berechtigungen pro rrennAIbook zuweisen.

### Impersonation

Lehrkräfte und Admins können einen Schüler imitieren, um dessen Fortschritt im Nur-Lese-Modus einzusehen. Dabei wird das rrennAIbook mit dem gespeicherten Zustand des Schülers geöffnet, ohne dass Änderungen möglich sind.
...
```
Schüler interagiert mit dem rrennAIbook
        ↓
Dexie-Hook erfasst Änderung als Event
        ↓
Events gebündelt (verzögert 2s / max 10s)
        ↓
POST /api/store/:rrennAIbookId/events
        ↓
...
Vollständiger Dexie-Export erstellt
        ↓
POST /api/store/:rrennAIbookId/snapshot
        ↓
...
|---|---|---|
| `GET` | `/api/store/:rrennAIbookId` | Rekonstruierten Zustand abrufen (Snapshot + Event-Replay). |
| `POST` | `/api/store/:rrennAIbookId/events` | Einen Batch von Events anhängen. Enthält `afterEventId` zur Konflikterkennung. |
| `POST` | `/api/store/:rrennAIbookId/snapshot` | Vollständige Zustandsüberschreibung. Ersetzt alle Events und Snapshots. |
