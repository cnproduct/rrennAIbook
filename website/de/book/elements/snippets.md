...
Du kannst in deinen Snippets auf die rrennAIbook-Konfiguration zugreifen, indem du z.B. `{{{ rrennAIbook.name }}}` verwendest.

## Beispiel

### Beispiel 1: Block
Hier ist ein einfaches Beispiel für ein Snippet zur Verwendung eines Protect-Elements mit
demselben Passwort und derselben ID in Ihrem rrennAIbook.
Das Snippet in `snippets/password.md.hbs`:
```md
:::protect{id="1" password="rrennAIbook" description="Das Passwort ist der Name dieses Projekts."}
{{{ content }}}
:::
{{#if hint}}
:::alert{info}
rrennAIbook ist das Passwort.
:::
{{/if}}
```
Das Markdown, das Sie in Ihr rrennAIbook einfügen müssen:
```md
:::Snippet{#password}
:smiley:
:::
