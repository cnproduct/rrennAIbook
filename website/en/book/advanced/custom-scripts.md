...
:::alert{warn}
Custom scripts are best used with `allowDangerousHtml` enabled. It is highly recommended to not target rrennAIbook elements and to only target custom elements. The classes we use for the rrennAIbook are not considered stable and might change in the future. Therefore, target rrennAIbook elements only with caution.
:::
...
## Using the rrennAIbook Store API

rrennAIbook provides a built-in Dexie database that allows you to store and retrieve custom data in the browser's IndexedDB. This is useful for persisting user preferences, progress tracking, or any other custom data that should survive page reloads.

### Custom Data Table

The rrennAIbook store includes a `custom` table specifically designed for your custom data needs. The table schema is:
...
Your custom data is automatically included when users export their rrennAIbook data using the `rrennAIbookExport()` function. Similarly, it will be restored when they use `rrennAIbookImport()`. This ensures your custom data is preserved across browser sessions and can be transferred between devices.
