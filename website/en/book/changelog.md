...
The entire release archive of rrennAIbook is available on [NPM](https://www.npmjs.com/package/rrennAIbook)

:::alert{info}

If you need a new feature, open an [issue](https://github.com/openpatch/rrennAIbook/issues) and let's discuss.
...
- Show version of the rrennAIbook in the console. This can be configured to "text" showing it under the Powered by rrennAIbook text or "tooltip" showing it as a tooltip when hovering the Powered by rrennAIbook text.
...
- The file watcher now ignores dotfiles (`.git`, `.rrennAIbook`) and `node_modules`, preventing unnecessary rebuild triggers.
...
- Updated Onlineide and Sqlide to integrated into the rrennAIbook store. This invalidates the state of all onlineide and sqlide instances, so your students have to start fresh.
- Move to an event driven architecture for rrennAIbook cloud. This is a breaking change and you have to reset your database.
...
- Fix rrennAIbook cloud integration not working correctly.
...
- Add rrennAIbook cloud support for managing students data in a central place.
- See the documentation for more information: [rrennAIbook Cloud](/configuration/cloud)  
...
- Typst can now load custom fonts. You can use the `@font` directive to load fonts from your rrennAIbook project. [Learn more](/elements/typst#custom-fonts) 
...
A new breadcrumb navigation feature has been added to help users understand their location within the rrennAIbook structure.

**Global Configuration** in `rrennAIbook.json`:
```json
{
  "breadcrumb": true
}
...
- More coherent styling across the rrennAIbook interface
...
Write and preview Typst documents directly in your rrennAIbook with a powerful interactive editor.
...
Your rrennAIbook now works even when JavaScript is disabled or unavailable, making it more accessible and reliable.
...
- Improve the new command and make it accessible via `npx create rrennAIbook` and in the vs code extension.
...
- Add new `custom` table to the rrennAIbook Dexie store for user-managed state persistence. Users can now store and retrieve arbitrary JSON data in the browser's IndexedDB using `store.custom.put()` and `store.custom.get()`. Custom data is automatically included in export/import functionality. [Learn more](/advanced/custom-scripts#using-the-rrennAIbook-store-api)
...
- **Page Layout Options**: Added three layout options for rrennAIbook pages:
...
- **Automatic Favicon and PWA Icon Generation**: When building a rrennAIbook project, if no `favicon.ico` exists and a `logo` is defined in `rrennAIbook.json`, a complete set of favicons and PWA assets are automatically generated (60+ files including Android icons, Apple touch icons, Apple startup images, and web manifest with full metadata). The system intelligently searches for logos in the root folder, book folder, or public folder, and uses metadata from `rrennAIbook.json` (name, description, theme color, language, author) to create a professional PWA-ready icon set.
...
- Allow syntax highlighting and copy code to be disabled for inline code blocks. For this you have to set the `bypassInline` options in your `rrennAIbook.json` to `true`.
...
- Share rrennAIbook pages with media included
...
:::tab{title="Fixed :bug:" id="fixed"}
  - CLI argument port was not used. Now it is working again. For example: `rrennAIbook dev --port 3000`
:::
...
- The h5p element allows you to embed H5P content directly into your rrennAIbook. H5P is an open-source platform for creating, sharing, and reusing interactive content. With the h5p element, you can easily integrate H5P content such as quizzes, interactive videos, and presentations into your rrennAIbook. [Learn more](/elements/h5p)
...
- Make rrennAIbook config accessible in snippets.
...
- The webide element allows you to embed a web-based integrated development environment (IDE) directly into your rrennAIbook. This feature is particularly useful for interactive coding tutorials and exercises, enabling users to write and run code within the book itself. [Learn more](/elements/webide)
...
- Fix tabs and collapsibles not rendering rrennAIbook elements.
...
- Add `trailingSlash` option. When this option is enabled only directories and `index.html` files will be produced, when building the rrennAIbook. [Learn more](/configuration/book)
