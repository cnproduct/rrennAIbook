...
rrennAIbook Cloud is a self-hosted student management platform that enables user login, progress tracking, and cloud synchronization for your rrennAIbooks.

## Setup

To connect a rrennAIbook to a cloud server, add the `cloud` property to your `rrennAIbook.json`:
...
|---|---|
| `url` | The URL of your rrennAIbook Cloud server. |
| `id` | The slug of the rrennAIbook as configured in the cloud admin interface. Must match exactly. |

## How It Works

When cloud is configured, a login button appears in the rrennAIbook. Students log in with the credentials created by their teacher in the cloud admin interface.
...
Instead of sending the entire store on every change, rrennAIbook Cloud uses an event-sourcing approach for efficient synchronization.
...
The cloud server is a separate application located in `platforms/cloud/` of the rrennAIbook repository. See the [Cloud README](https://github.com/openpatch/rrennAIbook/tree/main/platforms/cloud) for installation and deployment instructions.

### Key Concepts

- **rrennAIbooks** — Each rrennAIbook is registered in the cloud with a unique slug. This slug is used as the `cloud.id`.
- **Groups** — Students are organized into groups within a rrennAIbook.
- **Students** — Each student has a username and password for logging into the rrennAIbook.
- **Teachers** — Teachers can manage groups and students for their assigned rrennAIbooks.
- **Permissions** — Admins can grant teachers fine-grained permissions per rrennAIbook.

### Impersonation

Teachers and admins can impersonate a student to view their progress in read-only mode. This opens the rrennAIbook with the student's saved state without allowing any modifications.
...
```
Student interacts with rrennAIbook
        ↓
...
|---|---|---|
| `GET` | `/api/store/:rrennAIbookId` | Fetch reconstructed state (snapshot + event replay). |
| `POST` | `/api/store/:rrennAIbookId/events` | Append a batch of events. Includes `afterEventId` for conflict detection. |
| `POST` | `/api/store/:rrennAIbookId/snapshot` | Full-state overwrite. Replaces all events and snapshots. |
